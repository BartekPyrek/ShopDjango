...
class Product(models.Model):
    ...
    def validate_unique(self, exclude=None):
        qs = Product.objects.filter(name=self.name)
        if qs.filter(name = self.name).exists():
            raise ValidationError("Nazwa produktu nie może się powtarzać.")
...
class Category(models.Model):
    ...
    def validate_unique(self, exclude=None):
        qs = Category.objects.filter(name=self.name)
        if qs.filter(name = self.name).exists():
            raise ValidationError("Nazwa kategorii nie może się powtarzać.")
